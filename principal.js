...
document.addEventListener("DOMContentLoaded", function(){
    ...
    function armarTabla(json) {
        ...
        html += '<option value="Sin definir" selected>Definir actividad</option></select><br>';
        html += '<div class="div-botones"><button id="btn-enviar">Crear personaje</button><br>';
        html += '<button id="btnCrear3Filas">Crear tres filas</button><br>';
        ...
        if (valorNacimiento=='value1'){
            for (let i=0; i<json.tpespecial.length; i++) {
                ...
                if (!((fechaNacimiento>1749)&&(fechaNacimiento<1800))) {
                    conteTabla.getElementsByTagName('tr')[i+1].classList.replace('verde', 'oculta');
                } else {
                    conteTabla.getElementsByTagName('tr')[i+1].classList.replace('oculta', 'verde');        
                } 
            }
